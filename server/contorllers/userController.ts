...
import type { ProjectIdParams } from "../types/index.js";
...
export const createUserProject = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await prisma.conversation.create({
        data : {
            role: 'assistant',
            content: `I've enhanced yor prompt to: "${}"`,
            projectId: project.id
        }
    })
    ...
    console.log(
      "Error from userControllers's createUserProject func. =",
      error.code || error.message
    );
    res.status(500).json({ message: error.message });
  }
...
export const getUserProject = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "Error from userControllers's getUserProject func. =",
      error.code || error.message
    );
    res.status(500).json({ message: error.message });
  }
...
export const getUserProjects = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "Error from userControllers's getUserProjects func. =",
      error.code || error.message
    );
    res.status(500).json({ message: error.message });
  }
...
export const togglePublish = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ message : project?.isPublished ? 'Project Unpublished' : 'Project Piblished Successfully' });

  } catch (error: any) {
    console.log(
      "Error from userControllers's togglePublish func. =",
      error.code || error.message
    );
    res.status(500).json({ message: error.message });
  }
...
export const purchaseCredits = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      "Error from userControllers's purchaseCredits func. =",
      error.code || error.message
    );
    res.status(500).json({ message: error.message });
  }
